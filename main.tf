...
resource "aws_instance" "this" {
  ...
  dynamic "instance_market_options" {
    ...
    content {
      market_type = "spot"
      spot_options {
        instance_interruption_behavior = try(var.spot_instance_interruption_behavior)
        max_price = try(var.spot_price)
        spot_instance_type = try(var.spot_type)
        valid_until = try(var.spot_valid_until)
      }
    }
  }
